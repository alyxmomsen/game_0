...
const recursiveCheck = (point: Cell, testCell: Cell) => {
  ...
  if (
    testCell.getPos().x + cellsDimestions.width > point.getPos().x &&
    testCell.getPos().x < point.getPos().x + cellsDimestions.width &&
    testCell.getPos().y + cellsDimestions.height > point.getPos().y && 
    testCell.getPos().y < point.getPos().y + cellsDimestions.height
  ) {
    return true ;
  }
  else {
    ...
    if(prevPoint) {
      ...
      recursiveCheck(prevPoint , testCell);
    }
    else {
      return false ;
    }
...
export default class MapManager {
  ...
  setCell() {
    const direction =
      MapManager.DIRECTIONS[
        Math.floor(Math.random() * MapManager.DIRECTIONS.length)
      ];
    // const cellsDimestions = Cell.getDimesions();
    ...
    const newPos: Position = {
      ...
      y:
        curPos.y + (direction === "down" ? 100 : direction === "up" ? -100 : 0),
    };
    ...
    if(recursiveCheck(this.cellIterator, testCell)) {
